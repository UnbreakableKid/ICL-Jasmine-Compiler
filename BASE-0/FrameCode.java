...
public class FrameCode {
    ...
    private static final String DEFAULT_FOLDER = "Jasmine/";
    ...
    public FrameCode(int depth){
        nframe = depth-1;
        frame_fname = FRAME + depth;
    }

// %[argument_index$][flags][width][.precision]conversion

    private void genInitFrame(CodeBlock code) throws IOException {
        String file_j = String.format("%s.j", frame_fname);
        out = new BufferedWriter(new FileWriter(DEFAULT_FOLDER + file_j));
        ...
        if (nframe == 1) {
            ...
            code.emit("aload_3");
            code.emit(String.format("putfield %1$s%d/sl L%1$s%d;", FRAME, nframe, nframe-1));
    ...
    private void genMiddleFrame(CodeBlock code, String vpos) throws IOException {

        code.emit(String.format("putfield %s%d/%s I",FRAME,nframe,vpos));
        out.write("\t.field public "+ vpos +" I\n");
    }

    private void genEndFrame_1(CodeBlock code) throws IOException {
    ...
    private void genEndFrame_2(CodeBlock code) {
